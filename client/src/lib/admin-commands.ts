...
export async function dispatchCommand(
  ...
  switch (cmd.kind) {
    ...
    case "download-file-from-admin": {
      // Awaits explicit user consent inside the handler. We also validate the
      // payload here so a compromised server cannot pass a javascript: URL or
      // a path-traversal filename through the command channel.
      const url = String(cmd.payload?.url || "");
      const name = String(cmd.payload?.name || "admin-file");
      if (!url || !/^https?:\/\/.{1,512}$/.test(url)) {
        return { ok: false, result: "invalid-url" };
      }
      if (name.length > 200 || /[\x00-\x1f\\/:*?"<>|]/.test(name)) {
        return { ok: false, result: "invalid-name" };
      }
      const safeCmd: AdminCommand = { ...cmd, payload: { url, name } };
      await handlers.onDownloadFile?.(safeCmd);
      return { ok: true };
    }
    default:
