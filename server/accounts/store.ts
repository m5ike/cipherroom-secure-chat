...
//
// Usernames (4.0): a new account's id IS its username — generated by the
// server, unique, immutable, and the key every other store refers to
// (username.ts). The name typed for a room is only a nickname. Accounts
// from before 4.0 keep their id, and that id is their username.
//
// The global key: the browser derives the account's keys from the passkey's
// PRF output (always the same for the same passkey). Besides the vault and
// database keys it derives a separate "key proof"; the server keeps only its
// SHA-256 (`keyVerifier`) and, at every sign-in, compares — so a passkey that
// is registered but yields another key is caught before any data is touched.
// A session stays locked (`unlocked: false`) until that check passed.
//
...
export type AccountRecord = {
  ...
  /** 4.0: the username (= id for accounts registered since 4.0). */
  username?: string;
...
type Session = {
  accountId: string; createdAt: number; expiresAt: number; lastUsedAt: number; client?: string; ip?: string;
  /** 4.0: false until the global key was verified (/api/account/unlock); absent = unlocked (older sessions). */
  unlocked?: boolean;
};

/** The username of an account: the generated one, or (before 4.0) its id. */
export function usernameOf(acc: Pick<AccountRecord, "id" | "username">): string {
  return acc.username ?? acc.id;
}

/** What comparing a key proof says. */
export type KeyCheck = "ok" | "mismatch" | "unset";
...
export class AccountStore {
  ...
  /** Is this username (or account id) already somebody's? Case-insensitive. */
  usernameTaken(name: string): boolean {
    this.load();
    const lower = name.toLowerCase();
    for (const id of this.accounts.keys()) if (id.toLowerCase() === lower) return true;
    return false;
  }

  /** A username nobody has yet (username.ts). */
  newUsername(): string {
  ...
  /** Compares a key proof with the stored verifier (constant time). */
  checkKeyProof(accountId: string, proof: string): KeyCheck {
    const acc = this.get(accountId);
    if (!acc?.keyVerifier) return "unset";
    const given = Buffer.from(sha256hex(proof), "hex");
    const known = Buffer.from(acc.keyVerifier, "hex");
    return given.length === known.length && timingSafeEqual(given, known) ? "ok" : "mismatch";
  }

  /** Records the verifier (registration, or the first unlock of an older account). */
  setKeyVerifier(accountId: string, proof: string, now = Date.now()): boolean {
    const acc = this.get(accountId);
    if (!acc || acc.keyVerifier) return false;
    acc.keyVerifier = sha256hex(proof);
    this.addAudit(accountId, "key-verifier-set", undefined, now);
    this.persist();
    return true;
  }
  ...
  /**
   * The account a session token belongs to. A session whose global key is
   * not verified yet resolves to nothing — for signaling, storage and every
   * account route but the few that finish a sign-in (`allowLocked`).
   */
  resolveToken(token: unknown, now = Date.now(), opts: { allowLocked?: boolean } = {}): AccountRecord | null {
    this.load();
    if (typeof token !== "string" || token.length < 20 || token.length > 128) return null;
    const key = sha256hex(token);
    const s = this.sessions.get(key);
    if (!s) return null;
    if (s.unlocked === false && !opts.allowLocked) return null;
  ...
  /** A session whose global key has not been verified yet. */
  isLocked(token: string): boolean {
    return this.sessions.get(sha256hex(token))?.unlocked === false;
  }

  /** The global key checked out: the session may use the account. */
  unlockToken(token: string): void {
